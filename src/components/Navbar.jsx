...
const Navbar = () => {
  ...
  return (
    <nav
      className={`${darkMode ? "bg-black text-gray-100" : "bg-white"} w-full`}
    >
      <div className="align-element py-4">
        ...
        <div className="md:hidden flex items-center justify-between">
          ...
          <div className="flex gap-6">
            ...
            {isOpen && (
              <div
                className={`fixed z-50 flex flex-col items-center font-semibold rounded-lg ${
                  darkMode
                    ? "bg-gray-950 text-gray-200"
                    : "text-slate-100 bg-blue-500"
                } gap-5 w-full mt-10 left-0 p-10`}
              >
                {links.map((link) => {
                  const { id, text } = link;
                  const route = text === "home" ? "/" : `/${text}`;
